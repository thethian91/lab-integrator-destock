...
from lab_core.maintenance import (
    backup,
    get_stats,
    purge,
    purge_all,
...
class MaintenanceTab(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.btn_purge_all.setStyleSheet(
        ...
        self.btn_purge_results.setStyleSheet(
    ...
    def refresh(self):
        ...
        state_str = (
            ", ".join([f"{k}:{v}" for k, v in s.get("by_status", {}).items()]) or "-"
        )
        self.lbl_by.setText(f"Por estado: {state_str}")

    def _pick_dir(self):
        d = QtWidgets.QFileDialog.getExistingDirectory(
            self, "Selecciona carpeta de backup"
        )
        if d:
    ...
    def _do_purge_results(self):
        ...
        try:
            n = purge_results()
            QtWidgets.QMessageBox.information(
                self, "Resultados", f"Resultados eliminados: {n}"
            )
        except Exception as e:
