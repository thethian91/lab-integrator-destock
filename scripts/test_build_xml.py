...
def run_single(db_path: str, result_id: int, outdir: str, quiet: bool = False) -> int:
    ...
    if not quiet:
        print(
            f"[test_xml] Generando {len(pairs)} XML(s) para result_id={result_id} → {out}"
        )
...
def run_range(
    ...
    if not triples:
        if not quiet:
            print(
                f"[test_xml] No se generaron XMLs para el rango {date_from}..{date_to}"
    ...
    if not quiet:
        print(
            f"[test_xml] Generando {len(triples)} XML(s) para rango {date_from}..{date_to}"
            + (f" (analyzer={analyzer})" if analyzer else "")
            + f" → {out}"
        )
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        "--db", default=DEFAULT_DB, help=f"Ruta de la BD SQLite (default: {DEFAULT_DB})"
    )
    ap.add_argument(
        "--out", default=DEFAULT_OUT, help=f"Carpeta de salida (default: {DEFAULT_OUT})"
    )
    ap.add_argument("--quiet", action="store_true", help="Menos salida en consola")

    mode = ap.add_mutually_exclusive_group(required=True)
    mode.add_argument(
        "--result-id", type=int, help="Generar XML(s) solo para este result_id"
    )
    ...
    ap.add_argument(
        "--analyzer", help="Filtrar por analyzer_name (opcional en modo rango)"
    )
...
def main() -> int:
    ...
    if args.result_id is not None:
        return run_single(
            db_path=args.db, result_id=args.result_id, outdir=args.out, quiet=args.quiet
        )
